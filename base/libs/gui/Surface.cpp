...
void Surface::setDirtyRectGroup(int *DirtyRectGroup, int DirtyRectNum)
{
    ...
    sp<ISurfaceTexture> st = getISurfaceTexture();
    if (st == 0) return ;
    ...
    for (int i = 0; i < DirtyRectNum; i +=5,DirtyRectGroup += 5)
    {
        ...
        {
            st->setDirtyRect(mLockedBufferIdx,DirtyRectGroup[0],DirtyRectGroup[1],
...
status_t Surface::lock(SurfaceInfo* other, Region* inOutDirtyRegion, int dirtyGroupSize, int * dirtyRectGroup) {
#else
...
int Surface::drainDisplayQueue() 
{
    return SurfaceTextureClient::drainDisplayQueue();
}
