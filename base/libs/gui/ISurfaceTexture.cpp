...
public:
    ...
    virtual status_t drainQueueLocked()  {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceTexture::getInterfaceDescriptor());
        status_t result = remote()->transact(DRAIN_QUEUE_LOCKED, data, &reply);
        if (result != NO_ERROR) {
            return result;
        }
        result = reply.readInt32();
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case DRAIN_QUEUE_LOCKED: {
            CHECK_INTERFACE(ISurfaceTexture, data, reply);
            bool lock = data.readInt32();
            status_t result = drainQueueLocked();
            reply->writeInt32(result);
            return NO_ERROR;
        } break;
