...
static jint Surface_drainDisplayQueue(JNIEnv* env, jobject clazz)
{
    const sp<Surface>& surface(getSurface(env, clazz));
    if (!Surface::isValid(surface))
        return 0;
            
    return surface->drainDisplayQueue();
        
}

static jint Surface_setDirtyRectGroup(JNIEnv* env, jobject clazz, jintArray dirtyRectGroup)
{
    ...
    if (dirtyRectGroup != NULL){
        ...
        if (dirtyRectGroupSize > 0)
        {
            ...
            surface->setDirtyRectGroup(pDirtyRectGroup, dirtyRectGroupSize);
...
static jobject Surface_lockCanvas(JNIEnv* env, jobject clazz, jobject dirtyRect, jintArray dirtyRectGroup)
{
    ...
    if (dirtyRectGroup != NULL){
        ...
        if (dirtyRectGroupSize > 0)
        {
            ...
            err = surface->lock(&info, &dirtyRegion, dirtyRectGroupSize, pDirtyRectGroup);
        }
        else
        {
            err = surface->lock(&info, &dirtyRegion, 0, NULL);
        }
    } else{
...
static JNINativeMethod gSurfaceMethods[] = {
    ...
    {"drainDisplayQueue", "()I", (void*)Surface_drainDisplayQueue },	
    {"lockCanvasNative",    "(Landroid/graphics/Rect;[I)Landroid/graphics/Canvas;",  (void*)Surface_lockCanvas },
    {"setDirtyRectGroup",   "([I)I", (void*)Surface_setDirtyRectGroup },
