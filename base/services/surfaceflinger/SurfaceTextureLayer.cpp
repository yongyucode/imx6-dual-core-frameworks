...
#ifdef FSL_EPDC_FB
void SurfaceTextureLayer::setDirtyRect(int index, int left, int top, int right, int bottom, int dirtyMode)
{
    sp<Layer> layer(mLayer.promote());
    if (layer != NULL) {
		layer->setDirtyRect(index, left, top, right, bottom, dirtyMode);
	}
}
#endif
// ---------------------------------------------------------------------------
