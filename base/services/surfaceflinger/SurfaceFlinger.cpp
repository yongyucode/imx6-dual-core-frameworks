...
void SurfaceFlinger::EinkOptPostFramebuffer()
{
            ...
            Region pInvalidRegion = mSwapRegion;
            Region lInvalidRegion = mSwapRegion;
            ...
            for (size_t i=0 ; i<count ; i++) {
                ...
                //LOGI("EinkOptPostFramebuffer mSwapRegion left=%d, top=%d, right=%d, bottom=%d \n",mSwapRegion.getBounds().left, mSwapRegion.getBounds().top,
                //              mSwapRegion.getBounds().right,mSwapRegion.getBounds().bottom);
            if ((mNeedspecialupdate > 0) ){
                    ...
                    for (size_t i=0 ; i<count ; i++) {
                        for (int j = 0; j < mReupdateDirtyReglist[i].mDirtyRegListLength; j ++)
                        {
                            ...
                            if (pDirtyRegNode != NULL)
                            {
                                mSwapRegion.subtractSelf(pDirtyRegNode->mDirtyRegion);
                            }
                        }
                    }
                    */
                    ...
                    for(int j=0; j < mSwapRegion.getRects(rectList); j++)
                    {
                ...
                else
                {
                    ...
                    postFramebuffer(mSwapRegion, allrectList,allupdatemode,1);
                }
...
void SurfaceFlinger::postFramebuffer(Region pInvalidRegion, Vector<Rect>& rectList, Vector<int>& modelist, int count)
{
    ...
    if (!pInvalidRegion.isEmpty()) {
        ...
        hw.flip(pInvalidRegion, secRotation, rectList, modelist, count);
        mLastSwapBufferTime = systemTime() - now;
        mDebugInSwapBuffers = 0;
    }
#else
    // this should never happen. we do the flip anyways so we don't
    // risk to cause a deadlock with hwc
    LOGW_IF(pInvalidRegion.isEmpty(), "pInvalidRegion is empty");
    const DisplayHardware& hw(graphicPlane(0).displayHardware());
    const nsecs_t now = systemTime();
    mDebugInSwapBuffers = now;
...
void SurfaceFlinger::composeSurfaces(const Region& dirty)
{
    ...
    for (size_t i=0 ; i<count ; i++) {
		...
		else
		{
			...
			const Region  exposedRegion = layer->visibleRegionScreen - coveredRegion;
...
void SurfaceFlinger::debugFlashRegions()
{
    ...
    allrectList.add(mSwapRegion.getBounds());
    allupdatemode.add(UI_DEFAULT_MODE);
    hw.flip(mSwapRegion, allrectList, allupdatemode, 1);
#else
    hw.flip(mSwapRegion);
#endif
    if (mDebugRegion > 1)
