...
{
    if (index < SurfaceTexture::NUM_BUFFER_SLOTS && index >= 0) {
        ...
        const Transform& planeTransform, Region& outDirtyRegion)
{
    ...
    if (!dirtyRegion.isEmpty()) {
        ...
        int buf = mSurfaceTexture->getCurrentSlotIndex();
        LOGW_IF((buf >= SurfaceTexture::NUM_BUFFER_SLOTS || buf < 0 ), " unlockPageFlip buf index is wrong\n");
        if (buf >=0 && buf < SurfaceTexture::NUM_BUFFER_SLOTS) {
...
{
    int buf = mSurfaceTexture->getCurrentSlotIndex();
    LOGW_IF((buf >= SurfaceTexture::NUM_BUFFER_SLOTS || buf < 0 ), " getCurrentDirtyRegList buf index is wrong\n");
    if (buf >= 0 && buf < SurfaceTexture::NUM_BUFFER_SLOTS) {
...
{
    int buf = mSurfaceTexture->getCurrentSlotIndex();
    LOGW_IF((buf >= SurfaceTexture::NUM_BUFFER_SLOTS || buf < 0 ), " finishPageFlip_eink buf index is wrong\n");
    if (buf >= 0 && buf < SurfaceTexture::NUM_BUFFER_SLOTS) {
